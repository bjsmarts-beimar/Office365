...
function SubmitFormWithValidation() {
    ...
    if ( !IsThisAValidDate("to")) {
        IsFormValid = false;
    }

    if ( !IsThisAValidDate("from")) {
        IsFormValid = false;
    }
    ...
    account.done(function (data) {
        ...
        accountRequestor.done(function (data) {
            ...
            record.done(function(data) {
                ...
                iaeBody = iaeBody.replace('{Title}', data.d.Title).replace('{ID}', data.d.ID).replace('{Supervisor}', SupervisorName).replace('{Supervisor Signature}', SupervisorName);
    
                let igaeTitle = igae.EmailSubject.replace('{Title}', data.d.Title);
                let igaeBody = stripHtml(igae.EmailBody);
                igaeBody = igaeBody.replace('{Title}', data.d.Title).replace('{ID}', data.d.ID).replace('{TypeID}', '2').replace('{Supervisor}', SupervisorName).replace('{Supervisor Signature}', SupervisorName);
