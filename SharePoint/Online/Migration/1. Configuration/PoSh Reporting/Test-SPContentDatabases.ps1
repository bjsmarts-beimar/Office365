...
#
Clear-Host
$outfile = ".\Reports\Test-SPContentDBs.csv"
$orphancheck = $true
...
foreach ($webapplication in $(Get-SPWebApplication))
{
    foreach ($contentdb in $webapplication.ContentDatabases)
    {
        ...
        Write-Host "Testing $($contentdb.Name)..." -NoNewline -ForegroundColor Yellow
        ...
        if ($orphancheck){
            ...
            Write-Host "orphan check..." -NoNewline -ForegroundColor Yellow
            [xml]$orphans = $contentdb.Repair($false)
            #$orphans.DocumentElement.OuterXml | Out-File "C:\Users\ruby\Desktop\orphans.xml"
        }
        Write-Host "`rTesting $($contentdb.Name) complete: " -NoNewline -ForegroundColor Yellow
        ...
        {
            Write-Host "No errors found" -ForegroundColor Green -NoNewline          
        ...
        else
        {
            ...
            {
                Write-Host "found $($orphans.OrphanedObjects.Count) orphans" -ForegroundColor Red -NoNewLine
            ...
            {
                Write-Host "found $($results.Count) errors " -ForegroundColor Red -NoNewLine
